 ...
 */
function PortalsContainer({ instance }) {
    ...
    <>
      {customResponseEvents.map(function mapEvent(event, index) {
        return (
          ...
          <ResponseComponentPortal key={index} hostElement={event.data.element}>
            ...
            <ResponseComponent message={event.data.message} />
          </ResponseComponentPortal>
        )
      })}
    </>
  )
 ...
 */
function ResponseComponentPortal({ hostElement, children }) {
